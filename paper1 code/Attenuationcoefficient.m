...
pixel_value= 0.0067;
AttenuationCoeff = zeros([size(image,1), size(image,2)]);

checkFlag = 0;
for i = 1: size(image,2)
    for j= 1: size(image,1)-1
        I= image(j,i);
        I1= log((1 + I/sum(image(j:end,i))));
        AttenuationCoeff(j,i) = I1/(2* pixel_value);
...
figure(2); imagesc(AttenuationCoeff); 
figure; imagesc(20*log10(abs(AttenuationCoeff))); colormap(gray)
New_image=(image - AttenuationCoeff);
figure(3); imagesc(New_image ,[40 120]);colormap(gray);
